...
local AilFields = {
  ...
  {50, 167, COMBO, 1, 4, { "KA1", "KA2", "KA3", "KA4", "KA5", "KA6", "KA7", "KA8" } }, -- Ail2 chan
}
...
local TailFields = {
  {50, 50, COMBO, 1, 1, { "En kanal för höjdroder, inget roder", "En kanal för höjdroder, en för sidroder", "Två kanaler för höjdroder, en för sidroder", "V-stjärt"} },
...
local function runTailConfig(event)
  ...
  lcd.drawText(40, 20, "Välj stjärtkonfiguration för din modell", COLOR_THEME_PRIMARY1)
  lcd.drawFilledRectangle(40, 45, 400, 30, TEXT_BGCOLOR)
  local result = runFieldsPage(event)
  return result
end
...
local function runConfigSummary(event)
  ...
  if(FlapsFields[1][5] == 1) then
    drawNextLine("Klaffkanal channel",FlapsFields[2][5])
  ...
  elseif (TailFields[1][5] == 3) then
    drawNextLine("Höger V-stjärt", TailFields[2][5])
    drawNextLine("Vänster V-stjärt", TailFields[3][5])
  end
  local result = runFieldsPage(event)
...
local function createModel(event)
  ...
  -- Flaps
  if(FlapsFields[1][5] == 1) then
    addMix(FlapsFields[2][5], MIXSRC_SA, "Klaff")
  elseif (FlapsFields[1][5] == 2) then
    addMix(FlapsFields[2][5], MIXSRC_SA, "KlaffH")
    addMix(FlapsFields[3][5], MIXSRC_SA, "KlaffV")
  end
  -- Tail
  if(TailFields[1][5] == 0) then
  ...
  elseif (TailFields[1][5] == 1) then
    addMix(TailFields[2][5], MIXSRC_FIRST_INPUT+defaultChannel(1), "Hjd")
    addMix(TailFields[3][5], MIXSRC_FIRST_INPUT+defaultChannel(0), "Rod")
  elseif (TailFields[1][5] == 2) then
