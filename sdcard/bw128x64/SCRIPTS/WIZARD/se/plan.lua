...
-- Ailerons Menu
local aileronsModeItems = {"Nej", "Ja, en kanal", "Ja, två kanaler"}
...
-- Flaps Menu
local flapsModeItems = {"Nej", "Ja, en kanal", "Ja, två kanaler"}
...
-- Airbrakes Menu
local brakesModeItems = {"Nej", "Ja, en kanal", "Ja, två kanaler"}
...
-- Tail Menu
local tailModeItems = {"Hjd(1)", "Hjd(1) + Sid(1)", "Hjd(2) + Sid(1)", "V-stjärt(2)"}
local function drawTailMenu()
  ...
  elseif tailMode == 1 then
    -- Elevator(1ch) + rudder...
    lcd.drawText(5, 30, "Ange kanal", 0);
    lcd.drawText(25, 40, "Hjd", 0);
    lcd.drawText(60, 40, "Sid", 0);
    lcd.drawText(5, 50, ">>>", 0);
  ...
  elseif tailMode == 2 then
    -- Elevator(2ch) + rudder...
    lcd.drawText(5, 30, "Ange kanal", 0);
    lcd.drawText(25, 40, "HjdV", 0);
    lcd.drawText(60, 40, "HjdH", 0);
    lcd.drawText(95, 40, "Sid", 0);
    lcd.drawText(5, 50, ">>>", 0);
    lcd.drawSource(25, 50, MIXSRC_CH1+eleCH1, getFieldFlags(1))
    ...
    -- V-Tail...
    lcd.drawText(5, 30, "Ange kanal", 0);
    lcd.drawText(25, 40, "VstjV", 0);
    lcd.drawText(60, 40, "VstjH", 0);
    lcd.drawText(5, 50, ">>>", 0);
    lcd.drawSource(25, 50, MIXSRC_CH1+eleCH1, getFieldFlags(1))
    lcd.drawSource(60, 50, MIXSRC_CH1+eleCH2, getFieldFlags(2))
    fieldsMax = 2
...
local function drawConfirmationMenu()
  ...
  if aileronsMode == 1 then
    x, y = drawNextLine(x, y, "Ske", ailCH1)
  elseif aileronsMode == 2 then
    x, y = drawNextLine(x, y, "SkeV", ailCH1)
    x, y = drawNextLine(x, y, "SkeH", ailCH2)
  end
  if flapsMode == 1 then
    x, y = drawNextLine(x, y, "Klf", flapsCH1)
  elseif flapsMode == 2 then
    x, y = drawNextLine(x, y, "KlfV", flapsCH1)
    x, y = drawNextLine(x, y, "KlfH", flapsCH2)
  end
  if brakesMode == 1 then
    x, y = drawNextLine(x, y, "Brms", brakesCH1)
  elseif brakesMode == 2 then
    x, y = drawNextLine(x, y, "BrmsV", brakesCH1)
    x, y = drawNextLine(x, y, "BrmsH", brakesCH2)
  end
  if tailMode == 3 then
    x, y = drawNextLine(x, y, "VstjV", eleCH1)
    x, y = drawNextLine(x, y, "VstjH", eleCH2)
  else
    x, y = drawNextLine(x, y, "Sid", rudCH1)
    if tailMode == 1 then
      x, y = drawNextLine(x, y, "Hjd", eleCH1)
    elseif tailMode == 2 then
      x, y = drawNextLine(x, y, "HjdV", eleCH1)
...
local function applySettings()
  ...
  if engineMode > 0 then
    addMix(thrCH1, MIXSRC_FIRST_INPUT+defaultChannel(2), "Motor")
  end
  if aileronsMode == 1 then
    addMix(ailCH1, MIXSRC_FIRST_INPUT+defaultChannel(3), "Ske")
  elseif aileronsMode == 2 then
    addMix(ailCH1, MIXSRC_FIRST_INPUT+defaultChannel(3), "SkeV", -100)
    addMix(ailCH2, MIXSRC_FIRST_INPUT+defaultChannel(3), "SkeH")
  end
  if flapsMode == 1 then
    addMix(flapsCH1, MIXSRC_SA, "Klf")
  elseif flapsMode == 2 then
    addMix(flapsCH1, MIXSRC_SA, "KlfV")
    addMix(flapsCH2, MIXSRC_SA, "KlfH")
  end
  if brakesMode == 1 then
    addMix(brakesCH1, MIXSRC_SD, "Brms")
  elseif brakesMode == 2 then
    addMix(brakesCH1, MIXSRC_SD, "BrmsV")
    addMix(brakesCH2, MIXSRC_SD, "BrmsH")
  end
  if tailMode == 3 then
    addMix(eleCH1, MIXSRC_FIRST_INPUT+defaultChannel(1), "V-HjdV", 50)
    addMix(eleCH1, MIXSRC_FIRST_INPUT+defaultChannel(0), "V-SidV", -50, 1)
    addMix(eleCH2, MIXSRC_FIRST_INPUT+defaultChannel(1), "V-HjdV", 50)
    addMix(eleCH2, MIXSRC_FIRST_INPUT+defaultChannel(0), "V-HjdH", 50, 1)
  else
    if tailMode > 0 then
      addMix(rudCH1, MIXSRC_FIRST_INPUT+defaultChannel(0), "Sida")
    end
    if tailMode == 1 then
      addMix(eleCH1, MIXSRC_FIRST_INPUT+defaultChannel(1), "Hjd")
    elseif tailMode == 2 then
      addMix(eleCH1, MIXSRC_FIRST_INPUT+defaultChannel(1), "HjdV")
      addMix(eleCH2, MIXSRC_FIRST_INPUT+defaultChannel(1), "HjdH")
    end
  end
end
