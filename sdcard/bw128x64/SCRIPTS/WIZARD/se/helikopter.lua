...
local function runTypeConfig(event)
  lcd.clear()
  fields = TypeFields
  lcd.drawScreenTitle("Helikoptertyp", 1,9)
  fields[2][4] = 0
  if fields[1][5] == 1 then
    lcd.drawText(0, 25, "Swashtyp")
    fields[2][4] = 1
  end
...
local StyleFields = {
  {0, 12, COMBO, 1, 0, { "Sport", "Lätt 3D","Full 3D" } },
}

local function runStyleConfig(event)
  lcd.clear()
  fields = StyleFields
  lcd.drawScreenTitle("Din flygstil",2,9)
  fields[1][4]=1
  local result = runFieldsPage(event)
  return result
end
...
local function runSwitchConfig(event)
  lcd.clear()
  lcd.drawScreenTitle("Ange brytare",3,9)
  fields = SwitchFields
  lcd.drawText(0, 12, "FL (Idla upp)")
  fields[1][4]=1
  lcd.drawText(0, 33, "Håll gas")
  fields[2][4]=1
  fields[3][4]=0
...
local function runThrConfig(event)
  lcd.clear()
  fields = ThrFields
  lcd.drawScreenTitle("Gaskanal",3,9)
  fields[1][4]=1
  local result = runFieldsPage(event)
  return result
end
...
local CurveFields = {
  {75, 12, COMBO, 1, 0, { "Gas upp", "V-kurva","Platt" } },
  {75, 32, COMBO, 1, 0, { "V-kurva","Platt" } },
  {75, 54, COMBO, 1, 0, { "V-kurva","Platt" } },
}

local function runCurveConfig(event)
  lcd.clear()
  fields = CurveFields
  lcd.drawScreenTitle("Gaskurvor flyglägen",4,9)
  lcd.drawText(0, 12, "Kurva FL0")
  fields[1][4]=1
  lcd.drawText(0, 32, "Kurva FL1")
  fields[2][4]=1
  lcd.drawText(0, 54, "Kurva FL2")
  fields[3][4]=1
  local result = runFieldsPage(event)
  return result
end
...
local function runAilerConfig(event)
  lcd.clear()
  fields = AilerFields
  lcd.drawScreenTitle("Skevroderkanal",5,9)
  fields[1][4]=1
  local result = runFieldsPage(event)
  return result
end
...
local function runEleConfig(event)
  lcd.clear()
  fields = EleFields
  lcd.drawScreenTitle("Höjdroderkanal",6,9)
  fields[1][4]=1
  local result = runFieldsPage(event)
  return result
end
...
local function runRudConfig(event)
  lcd.clear()
  fields = RudFields
  lcd.drawScreenTitle("Sidroderkanal (stjärt)",7,9)
  fields[1][4]=1
  local result = runFieldsPage(event)
  return result
end
...
local function drawNextChanelLine(text, text2)
  ...
  lcd.drawText(55, lineIndex, ": KA" .. text2 + 1)
...
local function runSummary(event)
  lcd.clear()
  lcd.drawScreenTitle("Sammanställning", 8,9)
  fields = SummaryFields
  lineIndex = 9

  -- Type
  if TypeFields[1][5]==0 then
    drawNextTextLine("TYP","FBL")
  else
    drawNextTextLine("TYP","FB")
  ...
  if StyleFields[1][5]==0 then
    drawNextTextLine("Stil","Sport")
  elseif StyleFields[1][5]==1 then
    drawNextTextLine("Stil","Lätt 3D")
  else
    drawNextTextLine("Stil","Full 3D")
  end

  -- Switch
  drawNextSwitchLine("Flygläge",SwitchFields[1][5])
  drawNextSwitchLine("Håll gas",SwitchFields[2][5])
  if TypeFields[1][5]==1 then
    drawNextSwitchLine("Gyro",SwitchFields[3][5])
  end

  -- thr
  drawNextChanelLine("Gas",ThrFields[1][5])
  fields[1][4]=1
  local result = runFieldsPage(event)
  return result
end
...
local function runConfigSummary(event)
  lcd.clear()
  lcd.drawScreenTitle("Lång [Enter] sparar", 9,9)
  fields = ConfigSummaryFields
  lineIndex = 8

  -- FM0 Curve
  if CurveFields[1][5]==0 then
    drawNextTextLine("Kurva FL0","Gasa upp")
  elseif CurveFields[1][5]==1 then
    drawNextTextLine("Kurva FL0","V-kurva")
  else
    drawNextTextLine("Kurva FL0 Curve","Platt")
  end

  -- FM1 Curve
  if CurveFields[2][5]==0 then
    drawNextTextLine("Kurva FL1","V-kurva")
  else
    drawNextTextLine("Kurva FL","Platt")
  end

  -- FM3 Curve
  if CurveFields[3][5]==0 then
    drawNextTextLine("Kurva FL2","V-kurva")
  else
    drawNextTextLine("Kurva FL2","Platt")
  end

  -- Ail
  drawNextChanelLine("Skev",AilerFields[1][5])

  -- Elev
  drawNextChanelLine("Höjd",EleFields[1][5])

  -- Rudder
  drawNextChanelLine("Sida",RudFields[1][5])
  fields[1][4]=1
  local result = runFieldsPage(event)
  return result
end
...
local function runCreateModel(event)
  ...
  model.setOutput(ThrFields[1][5],{name="Gas"})
  
  -- Ail
  if TypeFields[1][5] == 0 then
    model.insertMix(AilerFields[1][5], 0,{name="Ske",weight=100})
    model.setOutput(AilerFields[1][5],{name="Skev"})
  else
    model.insertMix(AilerFields[1][5], 0,{source=83,name="Ske",weight=100})
    model.setOutput(AilerFields[1][5],{name="Skev"})
  end

  -- Elev
  if TypeFields[1][5] == 0 then
    model.insertMix(EleFields[1][5], 0,{name="Hjd",weight=100})
    model.setOutput(EleFields[1][5],{name="Höjd"})
  else
    model.insertMix(EleFields[1][5], 0,{source=82,name="Hjd",weight=100})
    model.setOutput(EleFields[1][5],{name="Höjd"})
  end

  -- Rudder
  model.insertMix(RudFields[1][5], 0,{name="Sid",weight=100})
   model.setOutput(RudFields[1][5],{name="Sida"})
...
local function run(event)
  ...
  elseif event == EVT_VIRTUAL_ENTER_LONG and page == #pages then
    ...
    lcd.drawText(0,15,"Modellen kapad!")
    return 2
